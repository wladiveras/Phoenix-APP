...
type AvatarColor =
    ...
    | "h-red"
    | "h-yellow"
type AvatarDotColor = undefined | "primary" | "success" | "info" | "warning" | "danger"
...
const props = defineProps({
    ...
    size: {
        ...
        validator: (value: AvatarSize) => {
            ...
            if ([undefined, "small", "medium", "large", "big", "xl"].indexOf(value) === -1) {
                console.warn(`V-Avatar: invalid "${value}" size. Should be small, medium, large, big, xl or undefined`)
                return false
            }
    ...
    dotColor: {
        ...
        validator: (value: AvatarDotColor) => {
            // The value must match one of these strings
            if ([undefined, "primary", "info", "warning", "danger", "grey"].indexOf(value) === -1) {
                console.warn(
...
<template>
    ...
    >
        <slot name="avatar">
            ...
            />
            <span v-else class="avatar is-fake" :class="[squared && 'is-squared', color && `is-${color}`]">
                <span>{{ initials }}</span>
            </span>
            <img
