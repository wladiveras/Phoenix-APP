    ...
    size: {
        ...
        validator: (value: IconBoxSize) => {
            ...
            if ([undefined, "small", "medium", "large", "big", "xl"].indexOf(value) === -1) {
                console.warn(`V-IconBox: invalid "${value}" size. Should be small, medium, large, big, xl or undefined`)
                return false
            }
...
<template>
    <div
        class="v-icon"
        :class="[size && 'is-' + size, color && 'is-' + color, rounded && 'is-rounded', bordered && 'is-bordered']"
    >
        <slot></slot>
    </div>
</template>
