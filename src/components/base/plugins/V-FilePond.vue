...
import { OptionTypes, create, supported, registerPlugin, getOptions } from "filepond"
    ...
    size: {
        ...
        validator: (value: FilePondSize) => {
            ...
            if ([undefined, "small", "tiny"].indexOf(value) === -1) {
                console.warn(`V-FilePond: invalid "${value}" size. Should be small, tiny or undefined`)
                return false
            }
...
<template>
    <div class="filepond-profile-wrap" :class="[props.size && `is-${props.size}`]">
