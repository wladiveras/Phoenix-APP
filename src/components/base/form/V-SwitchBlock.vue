...
type SwitchBlockColor = undefined | "primary" | "info" | "success" | "warning" | "danger"
...
const props = defineProps({
    ...
    color: {
        ...
        validator: (value: SwitchBlockColor) => {
            ...
            if ([undefined, "primary", "info", "success", "warning", "danger"].indexOf(value) === -1) {
                console.warn(
...
<template inherit-attrs="false">
    <div :class="[label && 'switch-block', thin && label && 'thin-switch-block']">
        <template v-if="thin">
            <label :for="blockSwitchId" class="thin-switch" :class="[color && `is-${color}`]">
                <input
                    :id="blockSwitchId"
                    :checked="modelValue"
                    class="input"
        ...
        <template v-else>
            <label :for="blockSwitchId" class="form-switch" :class="[color && `is-${color}`]">
                <input
                    :id="blockSwitchId"
                    :checked="modelValue"
                    type="checkbox"
