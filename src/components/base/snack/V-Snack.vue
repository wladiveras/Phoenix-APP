...
type SnackColor = undefined | "primary" | "success" | "info" | "warning" | "danger"
...
const props = defineProps({
    ...
    color: {
        ...
        validator: (value: SnackColor) => {
            ...
            if ([undefined, "primary", "success", "info", "warning", "danger"].indexOf(value) === -1) {
                console.warn(
    ...
    size: {
        ...
        validator: (value: SnackSize) => {
            ...
            if ([undefined, "small"].indexOf(value) === -1) {
                console.warn(`V-Snack: invalid "${value}" size. Should be small or undefined`)
                return false
            }
...
<template>
    <div class="snack" :class="[white && 'is-white', size && `is-${size}`]">
        <div v-if="icon" class="snack-media is-icon" :class="[color && `is-${color}`, solid && `is-solid`]">
        ...
        <div v-else-if="image" class="snack-media">
            <img class="avatar" :src="image" alt="" @error.once="$event.target.src = placeholder" />
        </div>
