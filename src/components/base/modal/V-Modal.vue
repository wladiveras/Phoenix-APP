...
const props = defineProps({
    ...
    size: {
        ...
        validator: (value: ModalAction) => {
            ...
            if ([undefined, "small", "medium", "large", "big"].indexOf(value) === -1) {
                console.warn(`V-Modal: invalid "${value}" size. Should be small, medium, large, big or undefined`)
                return false
            }
    ...
    actions: {
        ...
        validator: (value: ModalAction) => {
            ...
            if ([undefined, "center", "right"].indexOf(value) === -1) {
                console.warn(`V-Modal: invalid "${value}" actions. Should be center, right or undefined`)
                return false
            }
...
<template>
    <teleport to="body">
        <div :class="[open && 'is-active', size && `is-${size}`]" class="modal v-modal">
            <div class="modal-background v-modal-close" @click="() => noclose === false && emit('close')"></div>
            <div class="modal-content">
                <div class="modal-card">
                    <header class="modal-card-head">
                        <h3>{{ title }}</h3>
                        <button class="v-modal-close ml-auto" aria-label="close" @click="emit('close')">
                    ...
                    <div
                        ...
                        :class="[actions === 'center' && 'is-centered', actions === 'right' && 'is-end']"
                    >
                        <slot name="cancel">
                            <a
