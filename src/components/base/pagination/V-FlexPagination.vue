...
<template>
    <nav role="navigation" class="flex-pagination pagination is-rounded" aria-label="pagination" data-filter-hide>
        <RouterLink v-if="lastPage > 1" :to="paginatedLink(currentPage - 1)" class="pagination-previous has-chevron">
            <i aria-hidden="true" class="iconify" data-icon="feather:chevron-left"></i>
        </RouterLink>
        <RouterLink v-if="lastPage > 1" :to="paginatedLink(currentPage + 1)" class="pagination-next has-chevron">
            <i aria-hidden="true" class="iconify" data-icon="feather:chevron-right"></i>
        </RouterLink>

        <ul class="pagination-list">
            <li v-if="showFirstLink">
                <RouterLink :to="paginatedLink(1)" class="pagination-link" aria-label="Goto page 1"> 1 </RouterLink>
            </li>
            ...
            <li v-if="showLastLink">
                <RouterLink :to="paginatedLink(lastPage)" class="pagination-link" :aria-label="`Goto page ${lastPage}`">
                    {{ lastPage }}
                </RouterLink>
            </li>
        </ul>
