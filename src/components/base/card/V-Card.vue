...
type CardColor = undefined | "primary" | "secondary" | "info" | "success" | "warning" | "danger"
...
const props = defineProps({
    radius: {
        ...
        validator: (value: CardRadius) => {
            ...
            if (["regular", "smooth", "rounded"].indexOf(value) === -1) {
                console.warn(`V-Card: invalid "${value}" radius. Should be smooth, rounded or regular`)
                return false
            }
    ...
    color: {
        ...
        validator: (value: CardRadius) => {
            // The value must match one of these strings
            if ([undefined, "primary", "secondary", "info", "success", "warning", "danger"].indexOf(value) === -1) {
                console.warn(
