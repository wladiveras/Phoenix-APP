    ...
    format: {
        ...
        validator: (value: MediaFormat) => {
            ...
            if (["4by3", "16by9"].indexOf(value) === -1) {
                console.warn(`V-CardMedia: invalid "${value}" format. Should be 16by9 or 4by3`)
                return false
            }
    ...
    <div class="card v-card">
        <div v-if="props.image" class="card-image">
            <figure class="image is-4by3" :class="[props.format && `is-${props.format}`]">
                <img :src="image" alt="" @error.once="$event.target.src = props.placeholder" />
            </figure>
        </div>
        <div class="card-content">
            <V-Block :title="props.title" :subtitle="props.subtitle" center narrow>
                <template #icon>
                    <V-Avatar v-if="props.avatar" :picture="props.avatar" :badge="props.badge" size="medium" />
                </template>
            </V-Block>
