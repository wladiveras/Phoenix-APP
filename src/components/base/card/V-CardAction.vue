    ...
    radius: {
        ...
        validator: (value: MediaCardRadius) => {
            ...
            if (["regular", "smooth", "rounded"].indexOf(value) === -1) {
                console.warn(`V-CardAction: invalid "${value}" radius. Should be regular, smooth or rounded`)
                return false
            }
...
<template>
    <div
        class="is-raised"
        :class="[radius === 'regular' && 's-card', radius === 'smooth' && 'r-card', radius === 'rounded' && 'l-card']"
    >
        <div class="card-head">
