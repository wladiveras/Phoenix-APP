...
const props = defineProps({
    ...
    size: {
        ...
        validator: (value: ProgressSize) => {
            ...
            if ([undefined, "tiny", "smaller", "small"].indexOf(value) === -1) {
                console.warn(`V-Progress: invalid "${value}" size. Should be tiny, smaller, small or undefined`)
                return false
            }
    ...
    color: {
        ...
        validator: (value: ProgressColor) => {
            // The value must match one of these strings
            if (["primary", "success", "info", "warning", "danger"].indexOf(value) === -1) {
                console.warn(
...
<template>
    <progress class="progress" :class="[size && `is-${size}`, color && `is-${color}`]" :value="value" :max="max">
        {{ value ? `${(value / max) * 100}%` : "" }}
    </progress>
</template>
